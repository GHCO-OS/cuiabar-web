...
import { getBridgeControlRequest, getBridgeRuntimeStatus, getPendingBridgeControlRequest, getWhatsAppAutomationSettings, getWhatsAppTestModeSettings, requestBridgeControl, resolveBridgeControlRequest, resolveCustomerCategory, setWhatsAppAutomationSettings, setWhatsAppTestModeSettings, updateBridgeRuntimeStatus, updateCustomerCategory, } from './control';
import { syncConversationToLocalCrm } from './crmAdapter';
import { acknowledgeOutboundDelivery, cancelAutomaticOutboundForSafety, createManualOutboundCommand, failOutboundDelivery, getOutboundCommandDetail, processInboundMessage, processStatusUpdate, pullPendingOutboundCommands, } from './service';
import { getConversationDetail, insertAuditLog, listConversationRows, openHandoff, updateConversation, } from './repository';
const requireAuthenticatedUser = (c) => {
...
const verifyInternalToken = (request, env) => {
...
export const registerWhatsAppOperationalRoutes = (app) => {
    ...
    app.get('/api/admin/whatsapp/overview', async (c) => {
        ...
        const openConversations = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_conversations WHERE status IN ('active', 'human_handoff')`));
        const handoffs = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_handoffs WHERE status IN ('open', 'claimed')`));
        const reservations = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_reservation_flows WHERE status IN ('collecting', 'ready', 'submitted')`));
        const pendingCommands = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_outbound_commands WHERE status IN ('pending', 'processing')`));
        return c.json({
            ok: true,
            metrics: {
    ...
    app.get('/api/admin/whatsapp/control-center', async (c) => {
        ...
        const [automation, bridge, bridgeControl, testMode, conversations, openConversations, handoffs, reservations, pendingCommands] = await Promise.all([
            ...
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_conversations WHERE status IN ('active', 'human_handoff')`)),
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_handoffs WHERE status IN ('open', 'claimed')`)),
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_reservation_flows WHERE status IN ('collecting', 'ready', 'submitted')`)),
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM whatsapp_outbound_commands WHERE status IN ('pending', 'processing')`)),
        ]);
    ...
    app.get('/api/admin/whatsapp/conversations/:id', async (c) => {
        ...
        return c.json({
            ...
            conversation: {
                ...
                tags: parseJsonText(detail.conversation.tags_json, []),
            ...
            profile: {
                ...
                tags: parseJsonText(detail.profile.tags_json, []),
