...
import { handoffTemplate, reservationAskGuestCountTemplate, reservationAskNameTemplate, reservationAskNotesTemplate, reservationAskTimeTemplate, reservationConfirmTemplate, reservationStartTemplate, reservationSuccessTemplate, } from './templates';
...
const parseTimeInput = (value, allowed) => {
    const normalized = sanitizeMessageText(value).toLowerCase();
...
const buildReservationSummary = (params) => [
...
const wantsCancel = (text) => CANCEL_WORDS.some((word) => sanitizeMessageText(text).toLowerCase().includes(word));
const wantsConfirm = (text) => CONFIRM_WORDS.includes(sanitizeMessageText(text).toLowerCase());
export const advanceReservationFlow = async (env, context, params) => {
    ...
    if (!params.currentFlow.customer_name ||
    ...
    const created = await createReservation(env, new Request('https://crm.cuiabar.com/api/internal/whatsapp/reservation', {
        headers: {
