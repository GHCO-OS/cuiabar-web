...
import { burgerTemplate, complaintTemplate, deliveryTemplate, eventTemplate, handoffTemplate, hoursTemplate, locationTemplate, marmitaTemplate, menuTemplate, unknownTemplate, unsupportedMessageTemplate, } from './templates';
const shouldInviteChannel = (session) => {
    if (!session.lastInviteAt) {
        return true;
    }
    return Date.now() - new Date(session.lastInviteAt).getTime() >= CHANNEL_INVITE_COOLDOWN_MS;
};
export const evaluateRules = async (env, context, params) => {
