...
const requireAuth = (user) => {
    if (!user)
        throw new HttpError(401, 'Autenticacao necessaria.');
    return user;
};
...
const mapContact = (r) => ({
    ...
    status: r.status,
    source: r.source,
    tags: parseJsonText(r.tags_json, []),
    customFields: parseJsonText(r.custom_fields_json, {}),
    notes: r.notes ?? '',
    ...
    aiProfile: parseJsonText(r.ai_profile_json, {}),
...
const mapMessage = (r) => ({
    ...
    metadata: parseJsonText(r.metadata_json, {}),
    createdAt: r.created_at,
});
const mapTemplate = (r) => ({
    id: r.id,
    ...
    category: r.category,
    content: r.content,
    variables: parseJsonText(r.variables_json, []),
    tags: parseJsonText(r.tags_json, []),
    language: r.language,
    status: r.status,
...
const mapSubscription = (r) => ({
    ...
    source: r.source,
    tags: parseJsonText(r.tags_json, []),
    customData: parseJsonText(r.custom_data_json, {}),
    status: r.status,
    optedInAt: r.opted_in_at,
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const suggestAiResponse = async (db, userMessage, sector) => {
    const rows = await all(db.prepare('SELECT * FROM wa_ai_training WHERE active = 1 ORDER BY usage_count DESC LIMIT 200'));
...
// REGISTRO DE ROTAS
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export const registerWhatsAppRoutes = (app) => {
    ...
    app.get('/api/crm/wa/dashboard', async (c) => {
        ...
        const [totalContacts, openConversations, todayMessages, aiUsage, newSubscriptions, sectors] = await Promise.all([
            first(db.prepare('SELECT COUNT(*) as total FROM wa_contacts WHERE status = ?').bind('ativo')),
            first(db.prepare("SELECT COUNT(*) as total FROM wa_conversations WHERE status = 'aberta'")),
            first(db.prepare("SELECT COUNT(*) as total FROM wa_messages WHERE created_at >= date('now')")),
            first(db.prepare('SELECT COUNT(*) as total FROM wa_messages WHERE ai_suggested = 1')),
            first(db.prepare("SELECT COUNT(*) as total FROM wa_subscriptions WHERE created_at >= date('now', '-7 days')")),
            all(db.prepare("SELECT sector, COUNT(*) as total FROM wa_contacts WHERE status = 'ativo' GROUP BY sector ORDER BY total DESC LIMIT 10")),
        ]);
        const recentConversations = await all(db.prepare(`
        ...
        LIMIT 5
      `));
        const trainingCount = await first(db.prepare('SELECT COUNT(*) as total FROM wa_ai_training WHERE active = 1'));
        return c.json({
            ok: true,
            metrics: {
    ...
    app.get('/api/crm/wa/contacts', async (c) => {
        ...
        if (tag) {
            where += ' AND json_extract(c.tags_json, ?) IS NOT NULL';
            binds.push(`$[*]`);
        }
        const [countRow, rows] = await Promise.all([
            first(db.prepare(`SELECT COUNT(*) as total FROM wa_contacts c WHERE ${where}`).bind(...binds)),
            all(db.prepare(`SELECT * FROM wa_contacts c WHERE ${where} ORDER BY c.updated_at DESC LIMIT ? OFFSET ?`).bind(...binds, limit, offset)),
        ]);
        return c.json({
            ok: true,
    ...
    app.get('/api/crm/wa/contacts/:id', async (c) => {
        requireAuth(c.get('user'));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(c.req.param('id')));
        if (!row)
            throw new HttpError(404, 'Contato nao encontrado.');
        const conversations = await all(c.env.DB.prepare('SELECT id, status, last_message_at, last_message_preview FROM wa_conversations WHERE contact_id = ? ORDER BY created_at DESC LIMIT 10').bind(row.id));
    ...
    app.post('/api/crm/wa/contacts', async (c) => {
        ...
        if (!body.phone)
            throw new HttpError(400, 'Telefone obrigatorio.');
        const existing = await first(c.env.DB.prepare('SELECT id FROM wa_contacts WHERE phone = ?').bind(body.phone));
        if (existing)
            throw new HttpError(409, 'Contato com este numero ja existe.');
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ativo', ?, ?)
    `).bind(id, body.phone, body.name ?? null, body.email ?? null, body.sector ?? 'geral', body.source ?? 'manual', asJson(body.tags ?? []), body.notes ?? null, body.birthday ?? null, body.address ?? null, body.optedIn ? 1 : 0, body.optedIn ? now : null, now, now));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(id));
        return c.json({ ok: true, contact: mapContact(row) }, 201);
    });
    app.put('/api/crm/wa/contacts/:id', async (c) => {
        requireAuth(c.get('user'));
        const body = await requireJsonBody(c.req.raw);
        const existing = await first(c.env.DB.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(c.req.param('id')));
        if (!existing)
            throw new HttpError(404, 'Contato nao encontrado.');
        const now = nowIso();
        await run(c.env.DB.prepare(`
      ...
      WHERE id = ?
    `).bind(body.name ?? null, body.email ?? null, body.sector ?? null, body.source ?? null, body.tags ? asJson(body.tags) : null, body.notes ?? null, body.status ?? null, body.optedIn !== undefined ? (body.optedIn ? 1 : 0) : null, body.birthday ?? null, body.address ?? null, body.loyaltyPoints ?? null, body.lifetimeValue ?? null, now, c.req.param('id')));
        const updated = await first(c.env.DB.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(c.req.param('id')));
    ...
    app.get('/api/crm/wa/conversations', async (c) => {
        ...
        const [countRow, rows] = await Promise.all([
            first(c.env.DB.prepare(`SELECT COUNT(*) as total FROM wa_conversations c LEFT JOIN wa_contacts ct ON ct.id = c.contact_id WHERE ${where}`).bind(...binds)),
    ...
    `).bind(c.req.param('id')));
        ...
        const messages = await all(c.env.DB.prepare(`
    ...
    app.post('/api/crm/wa/conversations', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        if (!body.contactId)
            throw new HttpError(400, 'contactId obrigatorio.');
        const contact = await first(c.env.DB.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(body.contactId));
        if (!contact)
            throw new HttpError(404, 'Contato nao encontrado.');
    ...
    app.put('/api/crm/wa/conversations/:id', async (c) => {
      ...
      UPDATE wa_conversations SET
        ...
        updated_at = ?
      WHERE id = ?
    `).bind(body.status ?? null, body.assignedToUserId ?? null, body.sentiment ?? null, body.pipelineStage ?? null, body.tags ? asJson(body.tags) : null, body.internalNotes ?? null, body.status ?? null, now, now, c.req.param('id')));
        return c.json({ ok: true });
    });
    ...
    app.post('/api/crm/wa/conversations/:id/messages', async (c) => {
        ...
        if (!body.content?.trim() && !body.mediaUrl)
            throw new HttpError(400, 'Conteudo ou midia obrigatoria.');
        const conv = await first(c.env.DB.prepare('SELECT id, contact_id FROM wa_conversations WHERE id = ?').bind(c.req.param('id')));
        if (!conv)
            throw new HttpError(404, 'Conversa nao encontrada.');
    ...
    `).bind(msgId, conv.id, conv.contact_id, body.type ?? 'texto', body.content ?? null, body.mediaUrl ?? null, body.aiSuggested ? 1 : 0, body.aiConfidence ?? null, body.templateId ?? null, body.isInternalNote ? 1 : 0, user.id, now));
    ...
    `).bind(now, (body.content ?? '').substring(0, 120), now, conv.id));
        ...
        const msg = await first(c.env.DB.prepare(`
    ...
    app.post('/api/crm/wa/inbound', async (c) => {
        ...
        if (!body.phone || !body.content)
            throw new HttpError(400, 'phone e content obrigatorios.');
        const db = c.env.DB;
        const now = nowIso();
        // â”€â”€ Modo Teste: bloqueia todos os nÃºmeros exceto o nÃºmero de teste â”€â”€
        const [testModeRow, testNumberRow] = await Promise.all([
            first(db.prepare("SELECT value_json FROM app_settings WHERE key = 'wa_test_mode'")),
            first(db.prepare("SELECT value_json FROM app_settings WHERE key = 'wa_test_number'")),
        ]);
        const testModeEnabled = testModeRow ? JSON.parse(testModeRow.value_json) === true : false;
        const testNumber = testNumberRow ? JSON.parse(testNumberRow.value_json) : '';
        if (testModeEnabled) {
        ...
        // upsert contact
        let contact = await first(db.prepare('SELECT * FROM wa_contacts WHERE phone = ?').bind(body.phone));
        if (!contact) {
      ...
      `).bind(cid, body.phone, body.name ?? null, body.sector ?? 'geral', now, now));
            contact = await first(db.prepare('SELECT * FROM wa_contacts WHERE id = ?').bind(cid));
        }
        else {
        ...
        let conv = await first(db.prepare("SELECT id FROM wa_conversations WHERE contact_id = ? AND status = 'aberta' ORDER BY created_at DESC LIMIT 1").bind(contact.id));
        if (!conv) {
    ...
    app.post('/api/crm/wa/ai-suggest', async (c) => {
        ...
        if (body.conversationId) {
            const conv = await first(c.env.DB.prepare('SELECT contact_id FROM wa_conversations WHERE id = ?').bind(body.conversationId));
            if (conv) {
                const contact = await first(c.env.DB.prepare('SELECT sector FROM wa_contacts WHERE id = ?').bind(conv.contact_id));
                if (contact)
    ...
    app.get('/api/crm/wa/ai-training', async (c) => {
        ...
        const [countRow, rows] = await Promise.all([
            first(c.env.DB.prepare(`SELECT COUNT(*) as total FROM wa_ai_training WHERE ${where}`).bind(...binds)),
            all(c.env.DB.prepare(`SELECT * FROM wa_ai_training WHERE ${where} ORDER BY usage_count DESC, created_at DESC LIMIT ? OFFSET ?`).bind(...binds, limit, offset)),
        ]);
        const categories = await all(c.env.DB.prepare("SELECT category, COUNT(*) as total FROM wa_ai_training WHERE active = 1 GROUP BY category ORDER BY total DESC"));
        return c.json({
            ok: true,
    ...
    app.post('/api/crm/wa/ai-training', async (c) => {
        ...
        if (!body.question?.trim() || !body.answer?.trim())
            throw new HttpError(400, 'Pergunta e resposta sao obrigatorias.');
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'manual', ?, ?, ?)
    `).bind(id, body.question.trim(), body.answer.trim(), body.category ?? 'geral', body.sector ?? 'geral', asJson(body.tags ?? []), body.confidenceScore ?? 1.0, user.id, now, now));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_ai_training WHERE id = ?').bind(id));
    ...
    app.put('/api/crm/wa/ai-training/:id', async (c) => {
      ...
      UPDATE wa_ai_training SET
        ...
        updated_at = ?
      WHERE id = ?
    `).bind(body.question ?? null, body.answer ?? null, body.category ?? null, body.sector ?? null, body.tags ? asJson(body.tags) : null, body.active !== undefined ? (body.active ? 1 : 0) : null, body.confidenceScore ?? null, now, c.req.param('id')));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_ai_training WHERE id = ?').bind(c.req.param('id')));
    ...
    app.get('/api/crm/wa/templates', async (c) => {
        ...
        const rows = await all(c.env.DB.prepare(`SELECT * FROM wa_templates WHERE ${where} ORDER BY usage_count DESC, name ASC`).bind(...binds));
    ...
    app.post('/api/crm/wa/templates', async (c) => {
        ...
        if (!body.name?.trim() || !body.content?.trim())
            throw new HttpError(400, 'Nome e conteudo obrigatorios.');
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 'ativo', 0, ?, ?, ?)
    `).bind(id, body.name.trim(), body.category ?? 'geral', body.content.trim(), asJson([...new Set([...vars, ...(body.variables ?? [])])]), asJson(body.tags ?? []), body.language ?? 'pt_BR', user.id, now, now));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_templates WHERE id = ?').bind(id));
    ...
    app.put('/api/crm/wa/templates/:id', async (c) => {
      ...
      UPDATE wa_templates SET
        ...
        updated_at = ?
      WHERE id = ?
    `).bind(body.name ?? null, body.category ?? null, body.content ?? null, vars ? asJson(vars) : null, body.tags ? asJson(body.tags) : null, body.status ?? null, now, c.req.param('id')));
        const row = await first(c.env.DB.prepare('SELECT * FROM wa_templates WHERE id = ?').bind(c.req.param('id')));
    ...
    app.get('/api/crm/wa/sectors', async (c) => {
        requireAuth(c.get('user'));
        const rows = await all(c.env.DB.prepare("SELECT * FROM wa_sectors WHERE active = 1 ORDER BY name ASC"));
        const counts = await all(c.env.DB.prepare("SELECT sector, COUNT(*) as total FROM wa_contacts GROUP BY sector"));
    ...
    app.post('/api/crm/wa/sectors', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        if (!body.name?.trim())
            throw new HttpError(400, 'Nome do setor obrigatorio.');
    ...
    app.get('/api/crm/wa/subscriptions', async (c) => {
        ...
        const [countRow, rows] = await Promise.all([
            first(c.env.DB.prepare(`SELECT COUNT(*) as total FROM wa_subscriptions WHERE ${where}`).bind(...binds)),
            all(c.env.DB.prepare(`SELECT * FROM wa_subscriptions WHERE ${where} ORDER BY opted_in_at DESC LIMIT ? OFFSET ?`).bind(...binds, limit, offset)),
        ]);
        return c.json({
            ok: true,
    ...
    app.post('/api/crm/wa/subscriptions', async (c) => {
        ...
        if (!body.phone?.trim())
            throw new HttpError(400, 'Telefone obrigatorio.');
        const existing = await first(c.env.DB.prepare('SELECT id, status FROM wa_subscriptions WHERE phone = ?').bind(body.phone));
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ativo', ?, ?, ?)
    `).bind(id, body.phone.trim(), body.name ?? null, body.email ?? null, body.sector ?? 'geral', body.source ?? 'formulario', asJson(body.tags ?? []), asJson(body.customData ?? {}), now, now, now));
        // also create/update wa_contact
        const existingContact = await first(c.env.DB.prepare('SELECT id FROM wa_contacts WHERE phone = ?').bind(body.phone));
        if (!existingContact) {
            const cid = generateId('wact');
            await run(c.env.DB.prepare(`
        INSERT INTO wa_contacts (id, phone, name, email, sector, source, tags_json, opted_in, opted_in_at, status, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, 'ativo', ?, ?)
      `).bind(cid, body.phone.trim(), body.name ?? null, body.email ?? null, body.sector ?? 'geral', body.source ?? 'formulario', asJson(body.tags ?? []), now, now, now));
        }
        else {
    ...
    app.get('/api/crm/wa/settings', async (c) => {
        ...
        const [rows, automationSettings, testModeSettings] = await Promise.all([
            all(db.prepare(`SELECT key, value_json FROM app_settings WHERE key IN (${keys.map(() => '?').join(',')})`).bind(...keys)),
    ...
    app.put('/api/crm/wa/settings', async (c) => {
        ...
        if (Object.prototype.hasOwnProperty.call(body, 'wa_test_mode') || Object.prototype.hasOwnProperty.call(body, 'wa_test_number')) {
            ...
            const nextPhone = Object.prototype.hasOwnProperty.call(body, 'wa_test_number')
        ...
        for (const [key, value] of Object.entries(body)) {
            if (!allowedKeys.includes(key) || key === 'wa_bot_enabled' || key === 'wa_test_mode' || key === 'wa_test_number')
                continue;
