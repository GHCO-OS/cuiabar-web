...
import { acknowledgeOutboundCommand, claimPendingOutboundCommands, cancelPendingOutboundCommands, createOutboundCommand, failOutboundCommand, findMessageByProviderId, getOpenReservationFlow, getOrCreateConversation, getOutboundCommand, getRecentTranscript, insertAuditLog, insertConversationMessage, markMessageProcessed, openHandoff, updateConversation, updateCustomerProfileSummary, updateMessageStatus, upsertCustomerProfile, upsertReservationFlow, } from './repository';
...
const updateSessionFromRule = (session, rule, contextChannelUrl) => {
...
const buildInteractionType = (hasHandoff, hasReservationFlow, intent) => hasHandoff ? 'handoff_opened' : hasReservationFlow || intent === 'reserva' ? 'reservation_message' : 'conversation_message';
const finalizeSuppressedInbound = async (params) => {
    ...
    const summary = (await summarizeConversationWithAi(params.env, context, transcript)) ??
...
export const processInboundMessage = async (env, message) => {
    ...
    const summary = (await summarizeConversationWithAi(env, context, transcript)) ??
        buildFallbackSummary({
            intent: rule.intent.intent,
...
export const processStatusUpdate = async (env, status) => {
    await updateMessageStatus(env, status.providerMessageId, status.status, status.rawPayload);
};
export const acknowledgeOutboundDelivery = async (env, params) => {
...
export const failOutboundDelivery = async (env, params) => {
...
export const pullPendingOutboundCommands = async (env, limit = 10) => {
    const automationSettings = await getWhatsAppAutomationSettings(env);
    const testModeSettings = await getWhatsAppTestModeSettings(env);
...
export const cancelAutomaticOutboundForSafety = async (env, params) => cancelPendingOutboundCommands(env, {
    reason: params.reason,
    sources: ['assistant'],
    excludedPhoneE164: params.excludedPhoneE164 ?? null,
});
export const createManualOutboundCommand = async (env, params) => {
...
export const getOutboundCommandDetail = (env, commandId) => getOutboundCommand(env, commandId);
