...
import type { BusinessContext, IntentResult, SessionState, WhatsAppIntent } from './types';
import { normalizeFreeText } from './utils';

const scoreIntent = (normalizedText: string, intent: WhatsAppIntent) => {
  if (intent === 'unknown') {
    return 0;
  }
...
const ruleBasedIntent = (normalizedText: string): IntentResult | null => {
  if (!normalizedText) {
    return null;
  }
...
export const detectIntent = async (env: Env, context: BusinessContext, session: SessionState, text: string): Promise<IntentResult> => {
