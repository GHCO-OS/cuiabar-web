...
const DEFAULT_IFOOD_STORE_URL = 'https://www.ifood.com.br/delivery/campinas-sp/villa-cuiabar--executivos--pratos-do-dia-jardim-aurelia/1af0e396-a7c8-46e1-b1a5-dd06486bb4ad';
...
const isCsrfExemptPath = (path) => CSRF_EXEMPT_PATHS.includes(path) || path.startsWith('/api/internal/whatsapp/');
const PERMISSIONS = {
    gerente: [
        'dashboard:read',
        'users:manage',
...
const parseAudienceFilter = (queryValue) => (queryValue ? queryValue.trim() : '');
...
const readSetting = async (env, key, fallback) => {
    const setting = await first(env.DB.prepare('SELECT value_json FROM app_settings WHERE key = ?').bind(key));
    return setting ? parseJsonText(setting.value_json, fallback) : fallback;
};
const writeSetting = async (env, key, value, userId) => {
    await run(env.DB.prepare(`INSERT INTO app_settings (key, value_json, updated_by_user_id, updated_at)
       VALUES (?, ?, ?, ?)
       ON CONFLICT(key) DO UPDATE SET value_json = excluded.value_json, updated_by_user_id = excluded.updated_by_user_id, updated_at = excluded.updated_at`).bind(key, asJson(value), userId ?? null, nowIso()));
};
const readGmailOAuthConnection = async (env) => {
    const primary = await readSetting(env, 'gmail_oauth_connection', null);
    if (primary?.refreshToken) {
        return primary;
    }
    const legacy = await readSetting(env, 'gmail_oauth_pending', null);
    return legacy?.refreshToken ? legacy : null;
};
const storeGmailOAuthConnection = async (env, connection) => {
    await writeSetting(env, 'gmail_oauth_connection', connection, null);
    await writeSetting(env, 'gmail_oauth_pending', connection, null);
};
const auditLog = async (env, request, userId, action, entityType, entityId, metadata = {}) => {
    await run(env.DB.prepare(`INSERT INTO audit_logs (id, user_id, action, entity_type, entity_id, metadata_json, ip, user_agent, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('audit'), userId, action, entityType, entityId, asJson(metadata), getRequestIp(request), request.headers.get('user-agent'), nowIso()));
};
const getSessionBundle = async (env, sessionId) => {
    if (!sessionId) {
        return { user: null, session: null };
    }
    const row = await first(env.DB.prepare(`SELECT
        s.id AS session_id,
        s.csrf_token,
        s.expires_at,
        u.id AS user_id,
       ...
       GROUP BY s.id, u.id`).bind(sessionId, nowIso()));
...
const setSessionCookies = (c, sessionId, csrfToken) => {
...
const clearSessionCookies = (c) => {
    deleteCookie(c, c.env.SESSION_COOKIE_NAME, { path: '/' });
    deleteCookie(c, c.env.CSRF_COOKIE_NAME, { path: '/' });
};
const getUserCount = async (env) => {
    const row = await first(env.DB.prepare('SELECT COUNT(*) AS total FROM users'));
    return row?.total ?? 0;
};
const isGoogleOnlyAuth = (env) => (env.AUTH_MODE ?? '').toLowerCase() === 'google_only';
const parseEmailSet = (value) => new Set((value ?? '')
    .split(',')
    .map((entry) => entry.trim().toLowerCase())
    .filter(Boolean));
const getConfiguredSenderName = (env) => {
...
const buildPartnerRedirectUrl = (request, targetUrl, defaults) => {
...
const storePublicInteraction = async (env, request, payload) => {
    ...
    const contact = normalizedEmail
        ? await first(env.DB.prepare('SELECT id FROM contacts WHERE email = ?').bind(normalizedEmail))
        : null;
    await run(env.DB.prepare(`INSERT INTO public_interaction_events
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('evt'), payload.eventId ?? null, payload.eventName, payload.eventCategory, payload.source, payload.channel ?? null, contact?.id ?? null, normalizedEmail, payload.identityPhone?.trim() || null, payload.sessionKey?.trim() || null, payload.externalRef?.trim() || null, payload.pagePath?.trim() || null, payload.pageLocation?.trim() || null, payload.href?.trim() || null, payload.label?.trim() || null, request.headers.get('referer'), getRequestIp(request), request.headers.get('user-agent'), asJson(payload.metadata ?? {}), nowIso()));
};
const createSession = async (env, request, userId) => {
    const sessionId = generateId('sess');
    const csrfToken = randomToken(16);
    const expiresAt = new Date(Date.now() + SESSION_DURATION_MS).toISOString();
    await run(env.DB.prepare(`INSERT INTO sessions (id, user_id, csrf_token, ip, user_agent, expires_at, created_at, last_seen_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(sessionId, userId, csrfToken, getRequestIp(request), request.headers.get('user-agent'), expiresAt, nowIso(), nowIso()));
    return { sessionId, csrfToken };
};
const ensureGoogleUser = async (env, email, name, subject, picture) => {
    ...
    const existing = await first(env.DB.prepare('SELECT id FROM users WHERE google_subject = ? OR email = ?').bind(subject, email));
    if (existing) {
        await run(env.DB.prepare(`UPDATE users
         SET email = ?, display_name = ?, google_subject = ?, avatar_url = ?, auth_provider = 'google', email_verified = 1, updated_at = ?
         WHERE id = ?`).bind(email, name, subject, picture ?? null, nowIso(), existing.id));
        return existing.id;
    }
    ...
    await run(env.DB.prepare(`INSERT INTO users
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'active', 'google', ?, ?, 1, ?, ?)`).bind(userId, email, placeholderPassword.hash, placeholderPassword.salt, placeholderPassword.iterations, name, subject, picture ?? null, nowIso(), nowIso()));
...
const getCampaign = async (env, campaignId) => {
    const campaign = await first(env.DB.prepare('SELECT * FROM campaigns WHERE id = ?').bind(campaignId));
    if (!campaign) {
        throw new HttpError(404, 'Campanha nao encontrada.');
    }
    return campaign;
};
const getTemplate = async (env, templateId) => {
    const template = await first(env.DB.prepare('SELECT * FROM templates WHERE id = ?').bind(templateId));
    if (!template) {
        throw new HttpError(404, 'Template nao encontrado.');
    }
    return template;
};
const getSuppressedEmails = async (env) => {
    const rows = await all(env.DB.prepare('SELECT email FROM suppression_list'));
    return new Set(rows.map((row) => normalizeEmail(row.email)));
};
const getContactsForList = async (env, listId) => all(env.DB.prepare(`SELECT c.* FROM contacts c
       JOIN contact_list_items cli ON cli.contact_id = c.id
       WHERE cli.list_id = ?`).bind(listId));
const evaluateSegmentCondition = async (env, contact, condition) => {
    ...
    if (condition.field === 'clicked_campaign_id' && typeof value === 'string') {
        const click = await first(env.DB.prepare(`SELECT COUNT(*) AS total
         FROM campaign_click_events
         WHERE campaign_id = ? AND contact_id = ?`).bind(value, contact.id));
        return (click?.total ?? 0) > 0;
    }
    if (condition.field === 'not_clicked_last_n_campaigns' && typeof value === 'number') {
        const campaigns = await all(env.DB.prepare(`SELECT id FROM campaigns
        ...
        const clicked = await first(env.DB.prepare(`SELECT COUNT(*) AS total
         FROM campaign_click_events
         WHERE contact_id = ? AND campaign_id IN (${campaigns.map(() => '?').join(',')})`).bind(contact.id, ...campaigns.map((entry) => entry.id)));
...
const getContactsForSegment = async (env, segmentId) => {
    const segment = await first(env.DB.prepare('SELECT rules_json FROM segments WHERE id = ?').bind(segmentId));
    if (!segment) {
        throw new HttpError(404, 'Segmento nao encontrado.');
    }
    const definition = parseJsonText(segment.rules_json, { match: 'all', conditions: [] });
    const contacts = (await all(env.DB.prepare('SELECT * FROM contacts'))).map((row) => mapContact(row));
...
const getAudienceContacts = async (env, campaign) => {
...
const ensureCampaignLinks = async (env, campaignId, html) => {
    const urls = extractHrefLinks(html);
    const links = [];
    for (const originalUrl of urls) {
        const hash = await sha256(originalUrl);
        let link = await first(env.DB.prepare('SELECT * FROM campaign_links WHERE campaign_id = ? AND original_url = ?').bind(campaignId, originalUrl));
        if (!link) {
            const id = generateId('link');
            await run(env.DB.prepare(`INSERT INTO campaign_links (id, campaign_id, original_url, tracking_path, url_hash, link_label, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(id, campaignId, originalUrl, `c/${id}`, hash, null, nowIso()));
            link = await first(env.DB.prepare('SELECT * FROM campaign_links WHERE id = ?').bind(id));
        }
        if (link) {
            links.push(link);
        }
...
const renderEmailForRecipient = async (env, campaign, template, recipient, contact) => {
...
const classifySendFailure = (message) => {
...
const refreshCampaignStats = async (env, campaignId) => {
    const recipientStats = await first(env.DB.prepare(`SELECT
        SUM(CASE WHEN status = 'sent' THEN 1 ELSE 0 END) AS total_sent,
        SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) AS total_failed,
        SUM(CASE WHEN status = 'unsubscribed' THEN 1 ELSE 0 END) AS total_unsubscribed
       FROM campaign_recipients
       WHERE campaign_id = ?`).bind(campaignId));
    const clickStats = await first(env.DB.prepare(`SELECT
    ...
    const observedRate = recipientStats?.total_sent
        ...
        : 0;
    await run(env.DB.prepare(`UPDATE campaigns
       SET total_sent = ?,
           total_failed = ?,
           total_unsubscribed = ?,
           total_click_events = ?,
           ...
           updated_at = ?
       WHERE id = ?`).bind(recipientStats?.total_sent ?? 0, recipientStats?.total_failed ?? 0, recipientStats?.total_unsubscribed ?? 0, clickStats?.total_click_events ?? 0, clickStats?.total_unique_clicks ?? 0, clickStats?.total_clicked ?? 0, observedRate, nowIso(), campaignId));
};
const queueCampaignRecipients = async (env, campaign) => {
    ...
    for (const contact of contacts) {
        await run(env.DB.prepare(`INSERT INTO campaign_recipients
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'queued', 0, ?, ?)`).bind(generateId('rcp'), campaign.id, contact.id, contact.email, contact.firstName || null, contact.lastName || null, asJson({ tags: contact.tags, source: contact.source }), randomToken(18), randomToken(18), nowIso(), nowIso()));
    }
    await run(env.DB.prepare(`UPDATE campaigns
       ...
       WHERE id = ?`).bind(contacts.length, nowIso(), campaign.id));
    return contacts.length;
};
const markContactSuppressed = async (env, email, reason, contactId, details = {}) => {
    await run(env.DB.prepare(`INSERT INTO suppression_list (id, contact_id, email, reason, source, details_json, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('sup'), contactId ?? null, email, reason, 'system', asJson(details), nowIso(), nowIso()));
};
const sendRecipient = async (env, campaign, template, recipient, request) => {
    const contact = await first(env.DB.prepare('SELECT * FROM contacts WHERE id = ?').bind(recipient.contact_id));
    if (!contact) {
    ...
    try {
        ...
        await run(env.DB.prepare(`UPDATE campaign_recipients
         SET status = 'sent', provider_message_id = ?, sent_at = ?, updated_at = ?, send_attempts = send_attempts + 1
         WHERE id = ?`).bind(gmailResult.id, nowIso(), nowIso(), recipient.id));
        await run(env.DB.prepare('UPDATE contacts SET last_sent_at = ?, updated_at = ? WHERE id = ?').bind(nowIso(), nowIso(), recipient.contact_id));
        await run(env.DB.prepare(`INSERT INTO send_events (id, campaign_id, recipient_id, contact_id, provider, provider_message_id, event_type, status, payload_json, created_at)
         VALUES (?, ?, ?, ?, 'gmail', ?, 'send', 'success', ?, ?)`).bind(generateId('send'), campaign.id, recipient.id, recipient.contact_id, gmailResult.id, asJson({ threadId: gmailResult.threadId }), nowIso()));
        if (request) {
        ...
        await run(env.DB.prepare(`UPDATE campaign_recipients
         ...
         WHERE id = ?`).bind(newStatus, message.slice(0, 500), nowIso(), recipient.id));
        await run(env.DB.prepare(`INSERT INTO send_events (id, campaign_id, recipient_id, contact_id, provider, event_type, status, error_code, error_message, payload_json, created_at)
         VALUES (?, ?, ?, ?, 'gmail', 'send', 'failed', ?, ?, ?, ?)`).bind(generateId('send'), campaign.id, recipient.id, recipient.contact_id, errorCode, message.slice(0, 500), asJson({}), nowIso()));
        if (errorCode === 'invalid_recipient') {
            await run(env.DB.prepare('UPDATE contacts SET status = ?, updated_at = ? WHERE id = ?').bind('bounced', nowIso(), recipient.contact_id));
            await markContactSuppressed(env, recipient.email_snapshot, 'sync_failure', recipient.contact_id, { message });
            await run(env.DB.prepare(`INSERT INTO bounce_events (id, campaign_id, recipient_id, contact_id, bounce_type, diagnosis, raw_json, observed_at, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('bounce'), campaign.id, recipient.id, recipient.contact_id, 'sync_failure', message.slice(0, 300), asJson({ message }), nowIso(), nowIso()));
        }
    }
};
const processCampaignQueue = async (env, request, campaignId) => {
    ...
    const dueScheduledCampaigns = await all(env.DB.prepare(`SELECT id FROM campaigns
       WHERE status = 'scheduled' AND scheduled_at IS NOT NULL AND scheduled_at <= ?
       ${campaignId ? 'AND id = ?' : ''}
       ORDER BY scheduled_at ASC`).bind(...(campaignId ? [nowIso(), campaignId] : [nowIso()])));
    for (const campaign of dueScheduledCampaigns) {
        await run(env.DB.prepare(`UPDATE campaigns
         SET status = 'sending', started_at = COALESCE(started_at, ?), updated_at = ?
         WHERE id = ?`).bind(nowIso(), nowIso(), campaign.id));
    }
    const activeCampaigns = await all(env.DB.prepare(`SELECT * FROM campaigns
       WHERE status = 'sending'
       ${campaignId ? 'AND id = ?' : ''}
       ORDER BY COALESCE(started_at, scheduled_at, created_at) ASC`).bind(...(campaignId ? [campaignId] : [])));
    const summaries = [];
    for (const campaign of activeCampaigns) {
        const template = await getTemplate(env, campaign.template_id);
        const sentLastMinute = await first(env.DB.prepare(`SELECT COUNT(*) AS total
         FROM send_events
         WHERE campaign_id = ? AND status = 'success' AND created_at >= ?`).bind(campaign.id, new Date(Date.now() - 60_000).toISOString()));
        ...
        const recipients = await all(env.DB.prepare(`SELECT * FROM campaign_recipients
         WHERE campaign_id = ? AND status = 'queued' AND send_attempts < ?
         ORDER BY created_at ASC
         LIMIT ?`).bind(campaign.id, retryLimit, batchLimit));
        for (const recipient of recipients) {
            await sendRecipient(env, campaign, template, recipient, request);
            if (campaign.send_pause_ms > 0)
                await new Promise((r) => setTimeout(r, campaign.send_pause_ms));
        }
        await refreshCampaignStats(env, campaign.id);
        const remaining = await first(env.DB.prepare(`SELECT COUNT(*) AS total FROM campaign_recipients WHERE campaign_id = ? AND status = 'queued' AND send_attempts < ?`).bind(campaign.id, retryLimit));
...
const parseTrackingToken = (token) => {
...
const unsubscribeByToken = async (env, request, token) => {
    const recipient = await first(env.DB.prepare('SELECT * FROM campaign_recipients WHERE unsubscribe_token = ?').bind(token));
    if (!recipient) {
    ...
    await run(env.DB.prepare(`UPDATE contacts
       ...
       WHERE id = ?`).bind(nowIso(), nowIso(), contact.id));
    await run(env.DB.prepare(`UPDATE campaign_recipients
       SET status = 'unsubscribed', unsubscribed_at = COALESCE(unsubscribed_at, ?), updated_at = ?
       WHERE id = ?`).bind(nowIso(), nowIso(), recipient.id));
    await markContactSuppressed(env, contact.email, 'unsubscribe', contact.id, { campaignId: recipient.campaign_id });
    await run(env.DB.prepare(`INSERT INTO unsubscribe_events (id, campaign_id, contact_id, recipient_id, token, request_ip, user_agent, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('unsub'), recipient.campaign_id, contact.id, recipient.id, token, getRequestIp(request), request.headers.get('user-agent'), nowIso()));
...
const buildDeliverabilityChecklist = async (env) => {
...
const gmailOauthSetupHtml = (env) => `<!doctype html>
<html lang="pt-BR">
  <head>
    <meta charset="utf-8" />
...
export const createApp = () => {
    ...
    app.post('/api/meta-conversions', async (c) => {
        ...
        await storePublicInteraction(c.env, c.req.raw, {
            ...
            eventCategory: body.event_name === 'PageView'
                ? 'page_view'
            ...
            channel: typeof customData.contact_channel === 'string'
            ...
            pageLocation: typeof customData.page_location === 'string'
    ...
    app.post('/api/public/contacts/capture', async (c) => {
        ...
        const existing = await first(c.env.DB.prepare('SELECT * FROM contacts WHERE email = ?').bind(email));
        const contactId = existing?.id ?? generateId('ctc');
        const tags = new Set([...(existing ? parseJsonText(existing.tags_json, []) : []), ...(body.tags ?? [])].map((tag) => tag.trim()).filter(Boolean));
        const optInStatus = body.optInStatus?.trim() || (body.consent ? 'confirmed' : 'pending');
        const source = body.source?.trim() || 'public_capture';
        if (existing) {
            await run(c.env.DB.prepare(`UPDATE contacts
           SET first_name = COALESCE(NULLIF(?, ''), first_name),
               ...
               END,
               updated_at = ?
           WHERE id = ?`).bind(body.firstName?.trim() || '', body.lastName?.trim() || '', body.phone?.trim() || '', source, asJson([...tags]), optInStatus, nowIso(), existing.id));
        }
        else {
            await run(c.env.DB.prepare(`INSERT INTO contacts
            (id, email, first_name, last_name, phone, source, tags_json, status, opt_in_status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`).bind(contactId, email, body.firstName?.trim() || null, body.lastName?.trim() || null, body.phone?.trim() || null, source, asJson([...tags]), optInStatus, nowIso(), nowIso()));
        }
        if (body.listId) {
            await run(c.env.DB.prepare(`INSERT OR IGNORE INTO contact_list_items (id, list_id, contact_id, created_at)
           VALUES (?, ?, ?, ?)`).bind(generateId('cli'), body.listId, contactId, nowIso()));
        }
        await storePublicInteraction(c.env, c.req.raw, {
    ...
    app.get('/api/auth/config', async (c) => c.json({
        ok: true,
    ...
    app.get('/oauth/gmail/start', (c) => {
        ...
        authUrl.searchParams.set('scope', 'openid email profile https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/calendar.events');
    ...
    app.get('/oauth/gmail/callback', async (c) => {
        ...
        if (!tokenPayload.refresh_token) {
            throw new HttpError(409, 'O Google nao retornou refresh token. Revogue o acesso anterior do app nesta conta e autorize novamente.', { email: authorizedEmail, scope: tokenPayload.scope ?? null });
        }
    ...
    app.post('/api/bootstrap/admin', async (c) => {
        ...
        const userId = generateId('usr');
        await run(c.env.DB.prepare(`INSERT INTO users (id, email, password_hash, password_salt, password_iterations, display_name, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)`).bind(userId, email, password.hash, password.salt, password.iterations, body.displayName.trim(), nowIso(), nowIso()));
    ...
    app.post('/api/auth/login', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        const email = ensureEmail(body.email);
        const user = await first(c.env.DB.prepare('SELECT * FROM users WHERE email = ?').bind(email));
    ...
    app.get('/api/auth/session', async (c) => c.json({
        ok: true,
    ...
    app.post('/api/auth/change-password', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        const row = await first(c.env.DB.prepare('SELECT password_hash, password_salt, password_iterations FROM users WHERE id = ?').bind(user.id));
        if (!row) {
        ...
        await run(c.env.DB.prepare(`UPDATE users SET password_hash = ?, password_salt = ?, password_iterations = ?, updated_at = ? WHERE id = ?`).bind(next.hash, next.salt, next.iterations, nowIso(), user.id));
        await auditLog(c.env, c.req.raw, user.id, 'auth.change_password', 'user', user.id, {});
        return c.json({ ok: true });
    });
    app.get('/api/users', async (c) => {
        requirePermission(c, 'users:manage');
        const rows = await all(c.env.DB.prepare(`SELECT u.id, u.email, u.display_name, u.status, u.created_at, u.last_login_at, GROUP_CONCAT(r.name) AS roles
         FROM users u
         LEFT JOIN user_roles ur ON ur.user_id = u.id
         LEFT JOIN roles r ON r.id = ur.role_id
         GROUP BY u.id
    ...
    app.post('/api/users', async (c) => {
        ...
        const userId = generateId('usr');
        await run(c.env.DB.prepare(`INSERT INTO users (id, email, password_hash, password_salt, password_iterations, display_name, status, auth_provider, email_verified, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?)`).bind(userId, email, password.hash, password.salt, password.iterations, body.displayName.trim(), isGoogleOnlyAuth(c.env) ? 'google' : 'local_password', isGoogleOnlyAuth(c.env) ? 1 : 0, nowIso(), nowIso()));
    ...
    app.put('/api/users/:id', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        await run(c.env.DB.prepare('UPDATE users SET display_name = ?, status = ?, updated_at = ? WHERE id = ?').bind(body.displayName.trim(), body.status, nowIso(), userId));
        await run(c.env.DB.prepare('DELETE FROM user_roles WHERE user_id = ?').bind(userId));
        await run(c.env.DB.prepare('INSERT INTO user_roles (user_id, role_id, created_at) VALUES (?, ?, ?)').bind(userId, body.role === 'gerente' ? 'role_manager' : 'role_marketing_operator', nowIso()));
        if (body.resetPassword) {
            requireStrongPassword(body.resetPassword);
            const password = await hashPassword(body.resetPassword);
            await run(c.env.DB.prepare(`UPDATE users SET password_hash = ?, password_salt = ?, password_iterations = ?, updated_at = ? WHERE id = ?`).bind(password.hash, password.salt, password.iterations, nowIso(), userId));
        }
    ...
    app.get('/api/contacts', async (c) => {
        ...
        const countRow = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM contacts c ${whereClause}`).bind(...params));
      ...
      LIMIT ? OFFSET ?`;
        const rows = await all(c.env.DB.prepare(sql).bind(...params, pageSize, offset));
        return c.json({
            ok: true,
    ...
    app.post('/api/contacts', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO contacts
          (id, email, first_name, last_name, phone, source, tags_json, status, opt_in_status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(contactId, email, body.firstName?.trim() || null, body.lastName?.trim() || null, body.phone?.trim() || null, body.source?.trim() || null, asJson(body.tags ?? []), body.status ?? 'active', body.optInStatus ?? 'confirmed', nowIso(), nowIso()));
    ...
    app.put('/api/contacts/:id', async (c) => {
        ...
        await run(c.env.DB.prepare(`UPDATE contacts
         ...
         WHERE id = ?`).bind(body.firstName?.trim() || null, body.lastName?.trim() || null, body.phone?.trim() || null, body.source?.trim() || null, asJson(body.tags ?? []), body.status ?? 'active', body.optInStatus ?? 'confirmed', nowIso(), contactId));
    ...
    app.get('/api/contacts/:id/history', async (c) => {
        requirePermission(c, 'contacts:read');
        const contactId = c.req.param('id');
        const sendHistory = await all(c.env.DB.prepare(`SELECT created_at, status, campaign_id, error_message
         FROM send_events
         WHERE contact_id = ?
         ORDER BY created_at DESC
         LIMIT 50`).bind(contactId));
        const clicks = await all(c.env.DB.prepare(`SELECT created_at, campaign_id, link_id
         FROM campaign_click_events
         WHERE contact_id = ?
         ORDER BY created_at DESC
         LIMIT 50`).bind(contactId));
        const unsubscribes = await all(c.env.DB.prepare(`SELECT created_at, campaign_id
         FROM unsubscribe_events
         WHERE contact_id = ?
         ORDER BY created_at DESC
         LIMIT 20`).bind(contactId));
        const publicInteractions = await all(c.env.DB.prepare(`SELECT created_at, event_name, event_category, source, channel, href, label
    ...
    app.post('/api/contacts/import', async (c) => {
        ...
        for (const [index, entry] of entries.entries()) {
            ...
            try {
                ...
                const existing = await first(c.env.DB.prepare('SELECT id FROM contacts WHERE email = ?').bind(email));
               ...
               SET first_name = COALESCE(NULLIF(?, ''), first_name),
                   ...
                   tags_json = ?,
                   updated_at = ?
               WHERE id = ?`).bind(firstName, lastName, phone, source, asJson([...tags]), nowIso(), existing.id));
                    skippedRows += 1;
                }
                else {
                    await run(c.env.DB.prepare(`INSERT INTO contacts
                (id, email, first_name, last_name, phone, source, tags_json, status, opt_in_status, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`).bind(contactId, email, firstName || null, lastName || null, phone || null, source || null, asJson([...tags]), optInStatus, nowIso(), nowIso()));
                    importedRows += 1;
                }
                if (body.listId) {
                    await run(c.env.DB.prepare(`INSERT OR IGNORE INTO contact_list_items (id, list_id, contact_id, created_at)
        ...
        await run(c.env.DB.prepare(`INSERT INTO csv_imports
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(importId, actor.id, 'upload.csv', entries.length, importedRows, skippedRows, errors.length, asJson(body.mapping), asJson(errors), nowIso()));
    ...
    app.get('/api/lists', async (c) => {
        ...
        const rows = await all(c.env.DB.prepare(`SELECT cl.id, cl.name, cl.description, cl.kind, cl.created_at, cl.updated_at, COUNT(cli.id) AS contact_count
    ...
    app.post('/api/lists', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO contact_lists (id, name, description, kind, created_by_user_id, created_at, updated_at)
         VALUES (?, ?, ?, 'static', ?, ?, ?)`).bind(listId, body.name.trim(), body.description?.trim() || null, actor.id, nowIso(), nowIso()));
        for (const contactId of body.contactIds ?? []) {
            await run(c.env.DB.prepare(`INSERT OR IGNORE INTO contact_list_items (id, list_id, contact_id, created_at)
           VALUES (?, ?, ?, ?)`).bind(generateId('cli'), listId, contactId, nowIso()));
        }
    ...
    app.put('/api/lists/:id', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        await run(c.env.DB.prepare('UPDATE contact_lists SET name = ?, description = ?, updated_at = ? WHERE id = ?').bind(body.name.trim(), body.description?.trim() || null, nowIso(), listId));
        if (body.contactIds) {
            await run(c.env.DB.prepare('DELETE FROM contact_list_items WHERE list_id = ?').bind(listId));
            for (const contactId of body.contactIds) {
                await run(c.env.DB.prepare(`INSERT INTO contact_list_items (id, list_id, contact_id, created_at)
             VALUES (?, ?, ?, ?)`).bind(generateId('cli'), listId, contactId, nowIso()));
            }
        }
    ...
    app.delete('/api/lists/:id', async (c) => {
        const actor = requirePermission(c, 'lists:write');
        const listId = c.req.param('id');
        const inUse = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM campaigns WHERE list_id = ? AND status NOT IN ('cancelled', 'failed', 'draft')`).bind(listId));
        if ((inUse?.total ?? 0) > 0) {
    ...
    app.get('/api/segments', async (c) => {
        ...
        const rows = await all(c.env.DB.prepare('SELECT * FROM segments ORDER BY created_at DESC'));
        return c.json({
            ok: true,
            segments: rows.map((row) => ({
                ...
                rules: parseJsonText(row.rules_json, { match: 'all', conditions: [] }),
                createdAt: row.created_at,
                updatedAt: row.updated_at,
            })),
        });
    ...
    app.post('/api/segments', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO segments (id, name, description, rules_json, created_by_user_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(segmentId, body.name.trim(), body.description?.trim() || null, asJson(body.rules), actor.id, nowIso(), nowIso()));
    ...
    app.put('/api/segments/:id', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        await run(c.env.DB.prepare(`UPDATE segments
         SET name = ?, description = ?, rules_json = ?, updated_at = ?
         WHERE id = ?`).bind(body.name.trim(), body.description?.trim() || null, asJson(body.rules), nowIso(), segmentId));
    ...
    app.delete('/api/segments/:id', async (c) => {
        const actor = requirePermission(c, 'segments:write');
        const segmentId = c.req.param('id');
        const inUse = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM campaigns WHERE segment_id = ? AND status NOT IN ('cancelled', 'failed', 'draft')`).bind(segmentId));
        if ((inUse?.total ?? 0) > 0) {
    ...
    app.get('/api/templates', async (c) => {
        ...
        const rows = await all(c.env.DB.prepare('SELECT * FROM templates ORDER BY updated_at DESC'));
        return c.json({
            ok: true,
            templates: rows.map((row) => ({
                ...
                variables: parseJsonText(row.variables_json, []),
                createdAt: row.created_at,
                updatedAt: row.updated_at,
            })),
        });
    ...
    app.post('/api/templates', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO templates
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(templateId, body.name.trim(), body.subject.trim(), body.preheader?.trim() || null, body.html, body.text, asJson(variables), actor.id, actor.id, nowIso(), nowIso()));
    ...
    app.put('/api/templates/:id', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        const variables = Array.from(new Set([...extractMergeVariables(body.html), ...extractMergeVariables(body.text), ...RESERVED_TEMPLATE_VARIABLES])).sort();
        await run(c.env.DB.prepare(`UPDATE templates
         SET name = ?, subject = ?, preheader = ?, html_content = ?, text_content = ?, variables_json = ?, updated_by_user_id = ?, updated_at = ?
         WHERE id = ?`).bind(body.name.trim(), body.subject.trim(), body.preheader?.trim() || null, body.html, body.text, asJson(variables), actor.id, nowIso(), templateId));
    ...
    app.delete('/api/templates/:id', async (c) => {
        const actor = requirePermission(c, 'templates:manage');
        const templateId = c.req.param('id');
        const inUse = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM campaigns WHERE template_id = ? AND status NOT IN ('cancelled', 'failed')`).bind(templateId));
        if ((inUse?.total ?? 0) > 0) {
    ...
    app.get('/api/campaigns', async (c) => {
        ...
        const pageSize = Math.min(200, Math.max(1, parseInt(c.req.query('pageSize') ?? '50', 10) || 50));
        const offset = (page - 1) * pageSize;
        const countRow = await first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM campaigns'));
        const total = countRow?.total ?? 0;
        const totalPages = Math.ceil(total / pageSize);
        const rows = await all(c.env.DB.prepare('SELECT * FROM campaigns ORDER BY created_at DESC LIMIT ? OFFSET ?').bind(pageSize, offset));
        return c.json({
            ok: true,
    ...
    app.get('/api/campaigns/:id', async (c) => {
        ...
        const recipients = await first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM campaign_recipients WHERE campaign_id = ?').bind(campaign.id));
        return c.json({
            ok: true,
            campaign: {
    ...
    app.post('/api/campaigns', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO campaigns
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(campaignId, body.name.trim(), body.subject.trim(), body.preheader?.trim() || null, body.templateId, body.segmentId ?? null, body.listId ?? null, body.fromName?.trim() || getConfiguredSenderName(c.env), body.fromEmail?.trim() || c.env.GMAIL_SENDER_EMAIL || c.env.DEFAULT_FROM_EMAIL, body.replyTo?.trim() || c.env.DEFAULT_REPLY_TO || null, body.scheduledAt ?? null, sendingSettings.batchSize, sendingSettings.ratePerMinute, sendingSettings.pauseMs, sendingSettings.campaignMaxRecipients, actor.id, nowIso(), nowIso()));
    ...
    app.put('/api/campaigns/:id', async (c) => {
        ...
        const body = await requireJsonBody(c.req.raw);
        await run(c.env.DB.prepare(`UPDATE campaigns
         SET name = ?, subject = ?, preheader = ?, template_id = ?, segment_id = ?, list_id = ?, from_name = ?, from_email = ?, reply_to = ?,
             ...
             updated_at = ?
         WHERE id = ?`).bind(body.name.trim(), body.subject.trim(), body.preheader?.trim() || null, body.templateId, body.segmentId ?? null, body.listId ?? null, body.fromName.trim(), body.fromEmail.trim(), body.replyTo?.trim() || null, body.scheduledAt ?? null, body.status ?? null, body.sendBatchSize ?? null, body.sendRatePerMinute ?? null, body.maxRecipients ?? null, nowIso(), campaignId));
    ...
    app.post('/api/campaigns/:id/launch', async (c) => {
        ...
        await run(c.env.DB.prepare(`UPDATE campaigns
         ...
         WHERE id = ?`).bind(nextStatus, body.scheduledAt ?? null, nextStatus, nowIso(), nowIso(), campaign.id));
    ...
    app.post('/api/campaigns/:id/cancel', async (c) => {
        const actor = requirePermission(c, 'campaigns:send');
        await run(c.env.DB.prepare(`UPDATE campaigns SET status = 'cancelled', finished_at = ?, updated_at = ? WHERE id = ?`).bind(nowIso(), nowIso(), c.req.param('id')));
    ...
    app.post('/api/campaigns/:id/duplicate', async (c) => {
        ...
        await run(c.env.DB.prepare(`INSERT INTO campaigns
          (id, name, subject, preheader, template_id, segment_id, list_id, from_name, from_email, reply_to, status,
           send_batch_size, send_rate_per_minute, send_pause_ms, max_recipients, created_by_user_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?)`).bind(newId, `${source.name} (copia)`, source.subject, source.preheader ?? null, source.template_id, source.segment_id ?? null, source.list_id ?? null, source.from_name, source.from_email, source.reply_to ?? null, source.send_batch_size, source.send_rate_per_minute, source.send_pause_ms, source.max_recipients, actor.id, nowIso(), nowIso()));
    ...
    app.get('/api/campaigns/:id/metrics', async (c) => {
        ...
        const campaign = await getCampaign(c.env, campaignId);
        const topLinks = await all(c.env.DB.prepare(`SELECT id, original_url, click_count_total, click_count_unique
         FROM campaign_links
         WHERE campaign_id = ?
         ORDER BY click_count_unique DESC, click_count_total DESC`).bind(campaignId));
        const recipients = await all(c.env.DB.prepare(`SELECT email_snapshot, status, sent_at, clicked_at, unsubscribed_at, last_error
         FROM campaign_recipients
         WHERE campaign_id = ?
         ORDER BY created_at ASC`).bind(campaignId));
        return c.json({
            ok: true,
    ...
    app.get('/api/campaigns/:id/progress', async (c) => {
        ...
        const campaignId = c.req.param('id');
        const campaign = await getCampaign(c.env, campaignId);
        const statusRows = await all(c.env.DB.prepare(`SELECT status, COUNT(*) as total FROM campaign_recipients WHERE campaign_id = ? GROUP BY status`).bind(campaignId));
        const counts = {};
    ...
    app.get('/api/reports/dashboard', async (c) => {
        ...
        const [campaigns, contacts, clicks, failures, unsubscribes, period] = await Promise.all([
            first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM campaigns')),
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM contacts WHERE status = 'active'`)),
            first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM campaign_click_events')),
            first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM send_events WHERE status = 'failed'`)),
            first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM unsubscribe_events')),
            all(c.env.DB.prepare(`SELECT substr(created_at, 1, 10) AS day, COUNT(*) AS total
           FROM send_events
           WHERE created_at >= ?
           GROUP BY substr(created_at, 1, 10)
           ORDER BY day ASC`).bind(new Date(Date.now() - 1000 * 60 * 60 * 24 * 14).toISOString())),
        ]);
        const totalSentRow = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM send_events WHERE status = 'success'`));
        return c.json({
            ok: true,
            metrics: {
    ...
    app.get('/api/audit-logs', async (c) => {
        ...
        const pageSize = Math.min(200, Math.max(1, parseInt(c.req.query('pageSize') ?? '50', 10) || 50));
        const offset = (page - 1) * pageSize;
        const countRow = await first(c.env.DB.prepare('SELECT COUNT(*) AS total FROM audit_logs'));
        const total = countRow?.total ?? 0;
        const totalPages = Math.ceil(total / pageSize);
        const rows = await all(c.env.DB.prepare(`SELECT a.*, u.display_name, u.email
         FROM audit_logs a
         LEFT JOIN users u ON u.id = a.user_id
         ORDER BY a.created_at DESC
         LIMIT ? OFFSET ?`).bind(pageSize, offset));
        return c.json({
            ok: true,
            logs: rows.map((row) => ({
                id: row.id,
    ...
    app.put('/api/settings/sending', async (c) => {
        ...
        const clamp = (key, min, max) => {
            ...
            if (body[key] !== undefined && (isNaN(v) || v < min || v > max))
                errors.push(`${key} deve ser entre ${min} e ${max}`);
        };
    ...
    app.get('/api/exports/:type', async (c) => {
        ...
        if (exportType === 'contacts.csv') {
            const rows = await all(c.env.DB.prepare('SELECT * FROM contacts ORDER BY created_at DESC'));
            return csvResponse(buildCsv(['id', 'email', 'first_name', 'last_name', 'phone', 'source', 'tags', 'status', 'opt_in_status', 'unsubscribed_at', 'created_at'], rows.map((row) => [row.id, row.email, row.first_name, row.last_name, row.phone, row.source, row.tags_json, row.status, row.opt_in_status, row.unsubscribed_at, row.created_at])), 'contacts.csv');
        }
        if (exportType === 'campaigns.csv') {
            const rows = await all(c.env.DB.prepare('SELECT * FROM campaigns ORDER BY created_at DESC'));
            return csvResponse(buildCsv(['id', 'name', 'subject', 'status', 'scheduled_at', 'started_at', 'finished_at', 'total_recipients', 'total_sent', 'total_failed', 'total_clicked', 'total_unsubscribed'], rows.map((row) => [row.id, row.name, row.subject, row.status, row.scheduled_at, row.started_at, row.finished_at, row.total_recipients, row.total_sent, row.total_failed, row.total_clicked, row.total_unsubscribed])), 'campaigns.csv');
        }
        if (exportType === 'clicks.csv') {
            const rows = await all(c.env.DB.prepare('SELECT * FROM campaign_click_events ORDER BY created_at DESC'));
            return csvResponse(buildCsv(['campaign_id', 'contact_id', 'link_id', 'request_ip', 'user_agent', 'referer', 'is_unique', 'created_at'], rows.map((row) => [row.campaign_id, row.contact_id, row.link_id, row.request_ip, row.user_agent, row.referer, row.is_unique, row.created_at])), 'clicks.csv');
        }
        if (exportType === 'unsubscribes.csv') {
            const rows = await all(c.env.DB.prepare('SELECT * FROM unsubscribe_events ORDER BY created_at DESC'));
            return csvResponse(buildCsv(['campaign_id', 'contact_id', 'request_ip', 'user_agent', 'created_at'], rows.map((row) => [row.campaign_id, row.contact_id, row.request_ip, row.user_agent, row.created_at])), 'unsubscribes.csv');
        }
        if (exportType === 'failures.csv') {
            const rows = await all(c.env.DB.prepare(`SELECT campaign_id, recipient_id, contact_id, error_code, error_message, created_at
           FROM send_events
           WHERE status = 'failed'
           ORDER BY created_at DESC`));
            return csvResponse(buildCsv(['campaign_id', 'recipient_id', 'contact_id', 'error_code', 'error_message', 'created_at'], rows.map((row) => [row.campaign_id, row.recipient_id, row.contact_id, row.error_code, row.error_message, row.created_at])), 'failures.csv');
        }
    ...
    app.get('/c/:token', async (c) => {
        ...
        const recipient = await first(c.env.DB.prepare('SELECT * FROM campaign_recipients WHERE tracking_token = ?').bind(trackingToken));
        if (!recipient) {
            throw new HttpError(404, 'Tracking nao encontrado.');
        }
        const link = await first(c.env.DB.prepare('SELECT * FROM campaign_links WHERE id = ? AND campaign_id = ?').bind(linkId, recipient.campaign_id));
        if (!link) {
            throw new HttpError(404, 'Link rastreavel nao encontrado.');
        }
        const recentDuplicate = await first(c.env.DB.prepare(`SELECT created_at FROM campaign_click_events
         WHERE recipient_id = ? AND link_id = ? AND created_at >= ?
         ORDER BY created_at DESC
         LIMIT 1`).bind(recipient.id, link.id, new Date(Date.now() - 30_000).toISOString()));
        if (!recentDuplicate) {
            const priorClick = await first(c.env.DB.prepare(`SELECT COUNT(*) AS total FROM campaign_click_events WHERE recipient_id = ? AND link_id = ?`).bind(recipient.id, link.id));
            const isUnique = (priorClick?.total ?? 0) === 0 ? 1 : 0;
            await run(c.env.DB.prepare(`INSERT INTO campaign_click_events
            (id, campaign_id, contact_id, recipient_id, link_id, request_ip, user_agent, referer, is_unique, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(generateId('clk'), recipient.campaign_id, recipient.contact_id, recipient.id, link.id, getRequestIp(c.req.raw), c.req.header('user-agent'), c.req.header('referer'), isUnique, nowIso()));
            await run(c.env.DB.prepare(`UPDATE campaign_links
           SET click_count_total = click_count_total + 1,
               click_count_unique = click_count_unique + ?
           WHERE id = ?`).bind(isUnique, link.id));
...
export const runScheduledWork = async (env) => {
