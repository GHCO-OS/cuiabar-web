...
import { DIETARY_RESTRICTION_LABELS, DISCOVERY_SOURCE_LABELS, MEAL_PERIOD_LABELS, SEATING_PREFERENCE_LABELS, } from './constants';
...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const describeGuestCount = (reservation) => reservation.guestCountMode === 'approximate' ? `Aproximadamente ${reservation.guestCount} pessoas` : `${reservation.guestCount} pessoas`;
const describeDietaryRestriction = (reservation) => reservation.dietaryRestrictionType === 'other' && reservation.dietaryRestrictionNotes
    ? `Outras (${reservation.dietaryRestrictionNotes})`
    : DIETARY_RESTRICTION_LABELS[reservation.dietaryRestrictionType];
const buildDetailRows = (reservation) => [
...
const buildHtmlBody = (reservation, intro) => {
    const detailRows = buildDetailRows(reservation)
        .map(([label, value]) => `<tr><td style="padding:10px 0;border-bottom:1px solid rgba(51,35,19,0.08);font-weight:600;color:#332313;vertical-align:top">${escapeHtml(label)}</td><td style="padding:10px 0;border-bottom:1px solid rgba(51,35,19,0.08);color:#5f6269">${escapeHtml(value)}</td></tr>`)
        .join('');
...
const buildTextBody = (reservation, intro) => ['Cuiabar Reservas', '', intro, '', ...buildDetailRows(reservation).map(([label, value]) => `${label}: ${value}`)].join('\n');
const getSenderEmail = (env) => env.GMAIL_SENDER_EMAIL || env.DEFAULT_FROM_EMAIL;
const getSenderName = (env) => env.GMAIL_SENDER_NAME || env.DEFAULT_FROM_NAME || 'Cuiabar';
const getReservationBaseUrl = (env) => env.RESERVATION_APP_BASE_URL || env.APP_BASE_URL || 'https://reservas.cuiabar.com';
export const sendRestaurantReservationNotification = async (env, reservation) => {
...
export const sendCustomerReservationCopy = async (env, reservation) => {
    ...
    return sendViaGmail(env, {
        ...
        html: buildHtmlBody(reservation, 'Sua reserva foi registrada com sucesso. Nossa equipe ja recebeu os detalhes e o codigo abaixo pode ser usado em qualquer contato de acompanhamento.'),
        text: buildTextBody(reservation, 'Sua reserva foi registrada com sucesso. Nossa equipe ja recebeu os detalhes e o codigo abaixo pode ser usado em qualquer contato de acompanhamento.'),
        listUnsubscribeUrl: getReservationBaseUrl(env),
        headers: {
            'X-Reservation-Code': reservation.reservationCode,
        },
