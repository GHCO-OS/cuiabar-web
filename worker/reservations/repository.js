...
const mapReservationRecord = (row) => ({
    id: row.id,
...
export const toPublicSummary = (record) => ({
...
export const findReservationByCode = async (env, reservationCode) => {
    const row = await first(env.DB.prepare('SELECT * FROM reservations WHERE reservation_code = ?').bind(reservationCode));
    return row ? mapReservationRecord(row) : null;
};
export const insertReservation = async (env, reservation) => {
    const timestamp = nowIso();
    await run(env.DB.prepare(`INSERT INTO reservations (
        id,
        reservation_code,
        reservation_date,
        reservation_time,
        ...
        created_at,
        updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(reservation.id, reservation.reservationCode, reservation.reservationDate, reservation.reservationTime, reservation.mealPeriod, reservation.customerFullName, reservation.reservationForType, reservation.reservedPersonName, reservation.guestCount, reservation.guestCountMode, reservation.hasChildren ? 1 : 0, reservation.dietaryRestrictionType, reservation.dietaryRestrictionNotes, reservation.seatingPreference, reservation.whatsappNumber, reservation.email, reservation.isExistingCustomer ? 1 : 0, reservation.discoverySource, reservation.notes, reservation.tolerancePolicyText, reservation.googleCalendarEventId ?? null, reservation.status ?? 'pending', reservation.requestIp ?? null, reservation.userAgent ?? null, timestamp, timestamp));
    return {
        ...reservation,
...
export const updateReservationCalendarEvent = async (env, reservationId, googleCalendarEventId) => {
...
export const updateReservationStatus = async (env, reservationId, status) => {
    await run(env.DB.prepare('UPDATE reservations SET status = ?, updated_at = ? WHERE id = ?').bind(status, nowIso(), reservationId));
};
export const insertReservationLog = async (env, reservationId, eventType, status, details) => {
    await run(env.DB.prepare(`INSERT INTO reservation_logs (id, reservation_id, event_type, status, details_json, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(generateId('rlog'), reservationId, eventType, status, asJson(details), nowIso()));
};
export const listReservations = async (env, filters) => {
    const statements = [];
    const bindings = [];
    ...
    const rows = await all(env.DB.prepare(`SELECT * FROM reservations ${whereClause} ORDER BY reservation_date ASC, reservation_time ASC, created_at DESC LIMIT 300`).bind(...bindings));
